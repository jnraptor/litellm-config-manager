...
def update_order_values(
    ...
    for model_entry in model_list:
        ...
        if current_order != order:
            ...
            model_id = litellm_params.get("model", "")
            logger.debug(
                f"Updated order for {model_name} ({model_id}): {current_order} → {order}"
            )
        else:
...
def generate_report(stats: Dict[str, int], logger: logging.Logger) -> None:
    ...
    logger.info(
        f"  - Unknown provider models (order 2): {stats['unknown_provider_models']}"
    )
    logger.info("=" * 60)
    ...
    try:
        ...
        if not args.dry_run:
            if stats["updated_models"] > 0:
                ...
                logger.info(
                    "✅ No changes needed - all order values are already correct"
                )
        else:
            logger.info(
